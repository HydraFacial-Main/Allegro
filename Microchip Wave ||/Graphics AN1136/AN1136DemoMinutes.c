...
//	DaysLeft = 30;
	...
	if((seconds & 0x80) == 0x80)
	{
		...
		date1 = minutes;	//to binary
		date1 = minutes & 0x0F;
		date1 += (minutes >> 4) * 10;
		oldMinutes = (WORD)date1;
	
		hours = ReadRTC(0x02);
		date1 = hours;	//to binary
		date1 = hours & 0x0F;
		date1 += (hours >> 4) * 10;
		oldMinutes += (WORD)(date1 * 60);
		   ...
		   //		if(buttonReleased == 0)
				{
			        ...
			        {
	    				buttonReleased = 1;
						if(hiddenButtonFlag == 1)	//second upper
						{
							hiddenButtonFlag = 0;
...
void __T2_ISR _T2Interrupt(void)	//100 ms
{
	...
	if((!continuousFlag) && (DaysLeft != 0))
	{
		...
		if(RTCtimer == 0)
		{
			...
			date1 = minutes & 0x0F;
			date1 += (minutes >> 4) * 10;
			newMinutes = (WORD)date1;
		
			hours = ReadRTC(0x02);
			date1 = hours;	//to binary
			date1 = hours & 0x0F;
			date1 += (hours >> 4) * 10;
			newMinutes += (WORD)(date1 * 60);

			if(newMinutes != oldMinutes)	//if not the same
			{
 ...
 /*
			...
			lastJulian |= ((DWORD)SST25ReadWord(ADDRESS_LAST_JULIAN_HIGH) << 16);

			if(lastJulian != julianDate)	//if not the same
			{
				difference = (BYTE)(actualTime - lastJulian);
...
*/				
	...
	if(hiddenButtonFlag)		//hidden button pressed?
	{
...
*/			
 ...
 /*
							...
							date1 = date & 0x0F;
							date1 += (date >> 4) * 10;
							julianDate = (DWORD)date1;

							date1 = month & 0x0F;
							date1 += (month >> 4) *10;
							julianDate += (DWORD)date1 * 31;

							date1 = year & 0x0F;
							date1 += (year >> 4) *10;
							julianDate += (DWORD)date1 * 12;


							julianDate += (DWORD)DaysLeft;
							...
							uitoa((julianDate - actualTime), (BYTE*)String);
